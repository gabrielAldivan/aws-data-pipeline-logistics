...
QUARANTINE_PATH = (
    f"s3://{args['SILVER_BUCKET']}/quarantine/freight/" if IS_GLUE
...
DQ_RULES: List[DQRule] = [
    DQRule("positive_weight",    "cargo_weight_tons > 0",           "error",
           "Cargo weight must be positive"),
    DQRule("non_negative_value", "freight_value_brl >= 0",          "error",
           "Freight value cannot be negative"),
    DQRule("non_negative_delay", "delay_minutes >= 0",              "error",
           "Delay in minutes cannot be negative"),
    DQRule("valid_departure",    "departure_time IS NOT NULL",       "error",
           "Departure time is required"),
    DQRule("valid_arrival",      "arrival_time IS NOT NULL",         "error",
           "Arrival time is required"),
    DQRule("time_sequence",      "arrival_time > departure_time",    "error",
           "Arrival must be after departure"),
    DQRule("reasonable_weight",  "cargo_weight_tons < 15000",        "warning",
           "Single trip > 15,000 tons is suspicious"),
    DQRule("known_cargo_type",
...
def cast_types(df: DataFrame) -> DataFrame:
    return (
        ...
        .withColumn("cargo_type",        F.upper(F.trim(F.col("cargo_type"))).cast(StringType()))
        .withColumn("origin_station",    F.upper(F.trim(F.col("origin_station"))).cast(StringType()))
        .withColumn("destination_station", F.upper(F.trim(F.col("destination_station"))).cast(StringType()))
    )
...
def derive_columns(df: DataFrame) -> DataFrame:
    return (
        ...
        .withColumn("trip_date",
                    F.to_date("departure_time"))
        .withColumn("departure_hour",
                    F.hour("departure_time"))
        .withColumn("trip_duration_hours",
                    F.round(
                        (F.unix_timestamp("arrival_time") - F.unix_timestamp("departure_time")) / 3600,
                        2
                    ))
        # Business KPIs
        .withColumn("revenue_per_ton",
                    F.when(F.col("cargo_weight_tons") > 0,
                           F.round(F.col("freight_value_brl") / F.col("cargo_weight_tons"), 2))
                    .otherwise(None))
        .withColumn("margin_brl",
                    F.round(F.col("freight_value_brl") - F.col("fuel_cost_brl"), 2))
        .withColumn("is_delayed",
                    (F.col("delay_minutes") > 30).cast("boolean"))
    )
...
def run():
    ...
    df = cast_types(df)
    df = df.filter(F.col("departure_time").isNotNull() & F.col("arrival_time").isNotNull())
    ...
    meta_cols = [c for c in df_clean.columns if c.startswith("_") and not c.startswith("_warn")]
